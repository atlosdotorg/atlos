defmodule PlatformWeb.Components do
  ...
  def update_entry(assigns) do
    ...
    if is_list(update) do
      ...
      ~H"""
      <li
        class={"transition-all " <> (if @update.hidden, do: "opacity-50", else: "")}
                  ...
                  <%= case @update.type do %>
                    <% :update_attribute -> %>
                      ...
                      <%= if not is_nil(attr) do %>
                        <.link class = "text-button text-gray-800 inline-block" patch = "/incidents/#{@update.media.slug}/history/#{attr.name}">
                          <%= attr.label %> &nearr;
                        </.link>
                      <% else %>
        ...
        <div class="py-2 sm:grid sm:grid-cols-3 sm:gap-4 -mb-2">
          ...
          <dd class="mt-1 flex flex-wrap gap-2 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
            <%= for attr <- @unset_attrs do %>
              <.link class = "button original" patch = {Routes.media_show_path(@socket, :edit, @media.slug, attr.name)} replace = {true}></.link>
            <% end %>
          </dd>
        </div>
      <% end %>
  ...
  def attr_display_row(assigns) do
    ...
    <div class="py-2 sm:grid sm:grid-cols-3 sm:gap-2">
      <dt class="text-sm font-medium text-gray-500 mt-1 flex justify-between items-center flex-wrap">
        ...
        <.link class = "text-button inline-block mb-1" patch = {Routes.media_show_path(@socket, :history, @media.slug, @attr.name)}>
          <.user_stack users={
            @updates
      ...
      <dd class="mt-1 flex items-center text-sm text-gray-900 sm:mt-0 sm:col-span-2">
        ...
        <span class="ml-4 flex-shrink-0">
          <%= if Permissions.can_edit_media?(@current_user, @media, @attr) and not @immutable do %>
            <.link method = "Update" class = "text-button mt-1 inline-block" patch = {Routes.media_show_path(@socket, :edit, @media.slug, @attr.name)} replace = {true} ></.link>
          <% end %>
        </span>
      </dd>
    </div>
