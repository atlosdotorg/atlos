defmodule PlatformWeb.Components do
  ...
  def media_card(%{media: %Media{} = media, current_user: %Accounts.User{} = user} = assigns) do
    ...
    ~H"""
      <a class="flex group flex-col items-center md:flex-row bg-white overflow-hidden shadow rounded-lg justify-between min-h-32 max-h-48" href={"/media/#{media.slug}"}>
  ...
  def user_stack(assigns) do
    ...
    <div class="flex -space-x-1 relative z-0 overflow-hidden">
      <%= for user <- @users |> Enum.take(5) do %>
        <img class="relative z-30 inline-block h-6 w-6 rounded-full ring-2 ring-white" src={Accounts.get_profile_photo_path(user)} title={user.username} alt={"Profile photo for #{user.username}"}>
      <% end %>
